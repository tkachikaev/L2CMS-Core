...
final class NewsHtmlSanitizer
{
    ...
    private const ALIGNABLE_ELEMENTS = [
        'p', 'h2', 'h3', 'h4', 'blockquote', 'figure',
    ];

    private const COLORS = [
        'default', 'gold', 'red', 'green', 'blue', 'muted',
    ];
    ...
    public function sanitize(string $html): string
    {
        ...
        try {
            ...
            $document->loadHTML(
                '<?xml encoding="UTF-8"><!doctype html><html><body><div id="l2forge-news-root">'.$html.'</div></body></html>',
                LIBXML_NONET | LIBXML_NOERROR | LIBXML_NOWARNING
            );

            $xpath = new DOMXPath($document);
            $root = $xpath->query('//*[@id="l2forge-news-root"]')->item(0);
