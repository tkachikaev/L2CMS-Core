...
<section class="account-detail-hero">
    ...
    @if ($canCreateAccount)<a wire:navigate.hover class="account-button secondary" href="{{ public_route('game-accounts.create') }}">{{ __('Create game account') }}</a>@endif
</section>
...
<section class="account-section">
    ...
    @forelse($worlds as $world)
        <article class="world-card">
            <header><div><span class="world-card-symbol" aria-hidden="true">◇</span><div><h3>{{ $world['server']->nameFor() }}</h3><p>{{ $world['server']->chronicle }} @if($world['server']->rates)· {{ $world['server']->rates }}@endif</p></div></div><span class="world-card-count">{{ count($world['characters']) }}</span></header>
            @if(!$world['available'])
