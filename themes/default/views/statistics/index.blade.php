            ...
            @if($servers->count() > 1)
                <nav class="statistics-server-switcher" aria-label="{{ __('Select game server') }}">
                    @foreach($servers as $serverOption)
                        <a @class(['active' => $serverOption->is($selectedServer)]) href="{{ public_route('statistics.show', ['gameServer' => $serverOption->id, 'section' => $activeSection]) }}">
        ...
        @else
            <nav class="statistics-tabs" aria-label="{{ __('Statistics sections') }}">
                @foreach($sections as $sectionKey => $sectionLabel)
                    <a @class(['active' => $activeSection === $sectionKey]) href="{{ public_route('statistics.show', ['gameServer' => $selectedServer->id, 'section' => $sectionKey]) }}">
                        {{ $sectionLabel }}
                    </a>
                @endforeach
            </nav>
