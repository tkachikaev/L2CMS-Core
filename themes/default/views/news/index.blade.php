...
<section class="container page-content">
    <div class="public-news-list">
        @forelse ($news as $item)
            <article class="panel public-news-card">
                <a class="public-news-cover" href="{{ news_url($item) }}" aria-label="{{ $item->titleFor() }}">
                    @if ($item->coverUrl())
                ...
                <div class="public-news-copy">
                    ...
                    <h2><a href="{{ news_url($item) }}">{{ $item->titleFor() }}</a></h2>
                    <p>{{ $item->excerptFor() }}</p>
                    <a class="public-news-more" href="{{ news_url($item) }}">{{ __('Read news →') }}</a>
                </div>
            </article>
        @empty
    ...
    @if ($news->hasPages())
        <div class="public-pagination">
            @if (! $news->onFirstPage())
                <a class="button button-ghost" href="{{ $news->previousPageUrl() }}">← {{ __('Back') }}</a>
            @endif
            <span>{{ __('Page :current of :last', ['current' => $news->currentPage(), 'last' => $news->lastPage()]) }}</span>
            @if ($news->hasMorePages())
                <a class="button button-ghost" href="{{ $news->nextPageUrl() }}">{{ __('Next') }} →</a>
            @endif
        </div>
    @endif
</section>
