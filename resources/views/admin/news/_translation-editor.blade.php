...
<div role="tabpanel" @class(['translation-panel', 'active' => $requiredLocale]) data-locale-panel="{{ $locale }}" @if(!$requiredLocale) hidden @endif>
    ...
    <div class="form-group">
        ...
        >
            ...
            <div id="body-editor-{{ $locale }}" class="rich-editor-canvas" contenteditable="true" role="textbox" aria-multiline="true" data-placeholder="{{ __('Start writing news…') }}">{!! $editorBody !!}</div>
            <textarea id="body_{{ $locale }}" name="translations[{{ $locale }}][body]" class="rich-editor-source" maxlength="200000" hidden>{{ $body }}</textarea>
