...
@section('title', __('Reward deliveries'))
@section('description', __('Read-only journal of web inventory transfers to game characters.'))
@section('content')
<div class="admin-overview audit-summary">
    ...
    <p class="admin-overview-copy">{{ __('Rewards are reserved before delivery. Known failures return them to the player inventory; unknown outcomes stay reserved for review.') }}</p>
</div>
...
<form class="admin-filter-form" method="GET" action="{{ route('admin.rewards.index') }}">
    ...
    <label><span>{{ __('Status') }}</span><select name="status"><option value="">{{ __('All statuses') }}</option>@foreach([\App\Models\RewardDelivery::STATUS_PENDING, \App\Models\RewardDelivery::STATUS_PROCESSING, \App\Models\RewardDelivery::STATUS_DELIVERED, \App\Models\RewardDelivery::STATUS_FAILED, \App\Models\RewardDelivery::STATUS_REVIEW] as $status)<option value="{{ $status }}" @selected($activeStatus === $status)>{{ \App\Models\RewardDelivery::statusLabelFor($status) }}</option>@endforeach</select></label>
    <button class="button button-secondary" type="submit">{{ __('Apply') }}</button>
</form>

@if($deliveries->isEmpty())
    <div class="admin-empty-state empty-state"><div class="empty-state-mark" aria-hidden="true">R</div><h2>{{ __('No reward deliveries yet') }}</h2><p>{{ __('Transfer operations will appear here after players send rewards to characters.') }}</p></div>
@else
    <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>{{ __('Date and time') }}</th><th>{{ __('Player') }}</th><th>{{ __('Server') }}</th><th>{{ __('Character') }}</th><th>{{ __('Rewards') }}</th><th>{{ __('Status') }}</th></tr></thead><tbody>
        @foreach($deliveries as $delivery)<tr>
            ...
            <td><span @class(['status-badge','status-badge-success'=>$delivery->status===\App\Models\RewardDelivery::STATUS_DELIVERED,'status-badge-danger'=>$delivery->status===\App\Models\RewardDelivery::STATUS_FAILED,'status-badge-warning'=>$delivery->status===\App\Models\RewardDelivery::STATUS_REVIEW])>{{ $delivery->statusLabel() }}</span>@if($delivery->failure_code)<code>{{ $delivery->failure_code }}</code>@endif</td>
        </tr>@endforeach
    </tbody></table></div>
