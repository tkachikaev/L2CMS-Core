...
@if($logs->isEmpty())
    <div class="empty-state"><div class="empty-state-mark" aria-hidden="true">J</div><h2>{{ __('No records yet') }}</h2><p>{{ __('New events appear here after sign-ins, content changes or system operations.') }}</p></div>
@else
    <div class="audit-table-wrap"><table class="audit-table"><thead><tr><th>{{ __('Date and time') }}</th><th>{{ __('Actor') }}</th><th>{{ __('Action') }}</th><th>{{ __('Target') }}</th><th>{{ __('Result') }}</th><th>{{ __('IP address') }}</th><th></th></tr></thead><tbody>
