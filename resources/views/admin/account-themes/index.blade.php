...
<div class="content-toolbar themes-toolbar">
    <div class="content-stat"><span>{{ __('Active account theme') }}</span><strong>{{ $activeThemeSlug ?: '—' }}</strong></div>
    <div class="content-stat"><span>{{ __('Installed') }}</span><strong>{{ count($themes) }}</strong></div>
    <div class="content-stat"><span>{{ __('Ready') }}</span><strong>{{ $validThemeCount }}</strong></div>
...
@if ($themes === [])
    <div class="empty-box">{!! __('No account themes found. Check the <code>account-themes</code> directory.') !!}</div>
@else
    <div class="theme-grid">
        @foreach ($themes as $theme)
            <article @class(['theme-card', 'active' => $theme['active'], 'invalid' => ! $theme['valid'] || ! $theme['compatible']])>
